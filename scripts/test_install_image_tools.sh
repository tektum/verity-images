...
syft_install=$(awk '/\/tmp\/syft .*\/usr\/local\/bin\// {print NR; exit}' "$script")
wolfi_exit=$(awk '/track" == wolfi/ {wolfi=1} wolfi && /^  exit$/ {print NR; exit}' "$script")
[[ -n "$apk_exit" && -n "$grype_install" && -n "$syft_install" && -n "$wolfi_exit" ]]
[[ "$apk_exit" -lt "$grype_install" && "$grype_install" -lt "$syft_install" ]]
[[ "$syft_install" -lt "$wolfi_exit" ]]
grep -Fx 'sudo install /tmp/copa /tmp/trivy /tmp/syft /usr/local/bin/' "$script"
...
grep -Fx '  SYFT_VERSION: 1.50.0' "$build_workflow"
grep -Fx '  SYFT_SHA256: bf7b29ff57f06da30918266a0e1c2885a8f99784798d1bdb1628886aa015d788' "$build_workflow"
