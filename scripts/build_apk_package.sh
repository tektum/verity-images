...
mapfile -t packages < <(find out/packages -type f -name 'openssl-fips-provider-3.1.2-r3.apk' -print | sort)
[[ "${#packages[@]}" -eq 1 ]]
mkdir artifact
cp "${packages[0]}" artifact/openssl-fips-provider-3.1.2-r3.apk
sha256sum artifact/openssl-fips-provider-3.1.2-r3.apk > artifact/SHA256SUMS

work_dir=$(mktemp -d)
trap 'rm -rf "$work_dir"' EXIT HUP INT TERM
mkdir "$work_dir/repository"
melange keygen "$work_dir/test.rsa"
cp artifact/openssl-fips-provider-3.1.2-r3.apk "$work_dir/repository/"
melange sign --signing-key "$work_dir/test.rsa" "$work_dir/repository/openssl-fips-provider-3.1.2-r3.apk"
melange index --arch "$architecture" --signing-key "$work_dir/test.rsa" \
  --output "$work_dir/repository/APKINDEX.tar.gz" \
  "$work_dir/repository/openssl-fips-provider-3.1.2-r3.apk"
