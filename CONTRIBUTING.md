...
closed for unknown, disabled, ambiguous, or unsupported inputs and records those
findings in its summary without hiding them. Patched streams preserve the exact
`build.yaml` rebuild route; if that cannot consume an operating-system fix, the
adjacent upstream digest remains Renovate's separate remediation path. A Go module
finding in a Wolfi recipe receives the same exact rebuild route only when that exact
recipe invokes `go/remediate`; other language-package findings remain blocked rather
than causing speculative source edits.
...
suppresses every action sharing that recipe or pure-APKO input set. It does not
suppress an independent flavor with different build inputs. A failed download,
...
version. One proposal supersedes an otherwise redundant exact rebuild for the same
stream. Every other local-package shape remains blocked and loud.
...
- `scripts/gen_apko_lock_targets.py` discovers every enabled Wolfi variant whose
  ...
  `<flavor>.melange.yaml`, because a recipe-backed variant resolves its lock
  during the build from an ephemerally signed local package. A pure APKO variant
  missing its committed lock, or holding a config that still needs build-time
  substitution, fails `lint`.
- `scripts/refresh_apko_locks.sh` regenerates each lock with the pinned apko from
  `scripts/install_image_tools.sh` against the signed repositories the config
  declares, and proposes it only when the regenerated bytes differ. Locks stay
  committed and reviewed; publication never resolves packages dynamically.
- One image gets one branch, `apko-lock/` plus its context with `/` replaced by
  `-`, such as `apko-lock/images-go-1.26`, and one pull request. Images are never
  combined and the automation never writes to `main`. Review the package delta like
  any other image pull request; merging rebuilds and republishes only that image.
- A refresh is a no-op only when the branch diff against `main` holds exactly the
  current lock paths with exactly the current contents and one open pull request
  exists, so an unchanged image is not rebuilt. Any other branch state, including a
  stale extra path left by an earlier delta, is reset by one fresh commit on the
