...
docker run --name "$container" --cpus 4 --read-only -d "$image" >/dev/null
wait_ready
...
docker run --name "$container" --cpus 4 --read-only -d \
...
grep -F 'A listener with port 0 must provide a Unix socket path' "$fixture/invalid.log" >/dev/null || fail 'invalid listener diagnostic not found'
