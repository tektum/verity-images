...
docker run --rm --user "$user" -e HOME=/work -e COSIGN_PASSWORD= -v "$tmp:/work" -w /work "$image" sign-blob --yes --tlog-upload=false --key cosign.key --output-signature blob.sig blob
